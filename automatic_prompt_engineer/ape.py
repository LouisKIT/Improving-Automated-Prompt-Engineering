...
def simple_prompts(dataset,
    ...
    prompt_gen_template = get_simple_prompt_gen_template(
        prompt_gen_template, prompt_gen_mode)
    conf = config.simple_config(
        eval_model, prompt_gen_model, prompt_gen_mode, n, eval_rounds, prompt_gen_batch_size, eval_batch_size)
    prompts, res, demo_fn = find_simple_prompts(eval_template, demos_template, dataset, dataset, conf, prompt_gen_template=prompt_gen_template)
    
    return prompts, res, demo_fn

def find_simple_prompts(eval_template,
                 demos_template,
                 prompt_gen_data,
                 eval_data,
                 conf,
                 ...
                 prompt_gen_template=None):
    """
    Function to generate prompts using APE that also returns the prompts in a list for further use.
    Parameters:
        eval_template: The template for the evaluation queries.
        demos_template: The template for the demos.
